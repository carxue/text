...
public class StreamTest {
    ...
    private static void streamMapToInt(Object obj) {
        /*map对于Stream中包含的元素使用给定的转换函数进行转换操作新生成的Stream只包含转换生成的元*/
        List<Integer> numbers = Arrays.asList(1, 6, 7, 4, null, 6, 7, 8, null, 10);
        numbers.stream().filter(a -> a != null).mapToInt(num -> num * 2).forEach(value -> {
            System.out.print(value + "   ");
        });
        System.out.println("=================skip=======================");
        /*skip对数据进行截取*/
        numbers.stream().filter(a -> a != null).mapToInt(num -> num * 2).skip(2).forEach(value -> {
            System.out.print(value + "   ");
        });
        System.out.println("==================distinct======================");
        /*distinct对重复原生进行过滤其实是调用对象的equall方法*/
        numbers.stream().filter(a -> a != null).distinct().mapToInt(num -> num * 2).skip(2).forEach(value -> {
            System.out.print(value + "   ");
        });
        System.out.println("==================limit======================");
        /*limit过滤需要展示的原生个数*/
        numbers.stream().filter(a -> a != null).distinct().mapToInt(num -> num * 2).skip(2).limit(3).forEach(value -> {
            System.out.print(value + "   ");
        });
