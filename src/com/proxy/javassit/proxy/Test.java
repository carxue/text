package com.proxy.javassit.proxy;
...
public class Test {
	...
	private static void createProxy() throws Exception {
		...
		// 动态创建代理类StationProxy在包中不存在通过makeClass生成
		CtClass cc = pool.makeClass("com.proxy.javassit.proxy.StationProxy");

		// 设置接口
		CtClass interface1 = pool.get("com.proxy.javassit.proxy.TicketService");
		// 继承接口
		cc.setInterfaces(new CtClass[] { interface1 });

		// 设置Field
		CtField field = CtField.make("private com.proxy.javassit.proxy.Station station;", cc);
		
		cc.addField(field);

		CtClass stationClass = pool.get("com.proxy.javassit.proxy.Station");
		...
		//创建收取手续费takeHandlingFee方法
		CtMethod takeHandlingFee = CtNewMethod.make("private void takeHandlingFee() {}", cc);
